...
describe("Operations blogs in general", () => {
  ...
  describe("CRUD operations on blogs endpoints", () => {
    ...
    test("Validation on invalid data", async () => {
    ...
    test("Error in creating a blog", async () => {
      const mockfn = jest.fn().mockRejectedValue(new Error("Database error"));
      jest
        .spyOn(cloudinary.uploader, "upload")
        .mockImplementationOnce(
          jest.fn().mockRejectedValue(new Error("Cloudinary Upload failure"))
        );
    ...
    test("Errors in updating a blog", async () => {
  ...
  describe("operations on comments endpoints", () => {
    ...
    test("deleting a comment on a blog", async () => {
      ...
      const fourthErrdeleteResult = await request(app)
        .delete(`/blogs/${testBlogId}/comments/63ac6153042cf31311085132`)
        .set("Authorization", `Bearer ${adminToken}`);
      expect(fourthErrdeleteResult.statusCode).toBe(404);
  ...
  describe("operations on likes endpoints", () => {
    ...
    test("like or unlike a blog", async () => {
      ...
      const resultAdmin = await request(app)
        ...
        .set("Authorization", `Bearer ${adminToken}`);
      expect(resultAdmin.statusCode).toBe(403);
...
describe("User endpoints", () => {
  ...
  test("Get a single user", async () => {
    const resultErrorId = await request(app).get("/users/ddfgsdsdusdfdysf");
    expect(resultErrorId.status).toBe(400);

    const resultNotFound = await request(app).get(
      "/users/63c821cead620158224066db"
    );
    ...
    const resultCorrect = await request(app).get(`/users/${userId}`);
    ...
    jest
      ...
      .mockImplementationOnce(
        jest
          .fn()
          .mockRejectedValue(new Error("Database error in getting a user"))
      );
    const resultServerErr = await request(app).get(`/users/${userId}`);
